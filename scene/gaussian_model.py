...
class GaussianModel:
    ...
    def capture(self, is_feature_model=False, is_style_model=False):
        ...
        if is_style_model:
            #print(self.style_transfer.state_dict())
            return (
                self._xyz,
                self._scaling,
                self._rotation,
                ...
                self.style_transfer.mlp,
                self.style_transfer.ode,
                self.style_transfer.rectified_flow_1,
                self.style_transfer.rectified_flow_2,
                self.style_transfer.rectified_flow_3,
            )
    ...
    def restore(self, model_args, training_args=None, from_feature_model=False, from_style_model=False):
        ...
        if from_style_model:
            #print(self.decoder_state_dict)
            #print(self.mlp_state_dict)
            ...
            #print(type(self.decoder_state_dict))
            #print(type(self.style_mlp))
            self.decoder = GaussianConv(self.get_xyz.detach()).cuda()
            self.decoder.load_state_dict(self.decoder_state_dict)
            #self.mlp.load_state_dict(self.mlp_state_dict)
            #state_dict = torch.load('/home/xueqi/Documents/CODE/StyleGaussian/decoder2_iter_160000.pth')

            #self.decoder.load_state_dict(state_dict)
            self.style_transfer = MulLayer().cuda()
            self.style_transfer.mlp=self.style_mlp
            self.style_transfer.ode=self.ode
            self.style_transfer.rectified_flow_1=self.flow1
            self.style_transfer.rectified_flow_2 = self.flow2
            self.style_transfer.rectified_flow_3 = self.flow3
    ...
    def training_setup_feature(self, training_args):
        ...
        l = [
            ...
            {'params': self.feature_linear.parameters(), 'lr': 1e-4, "name": "feature_linear"}
        ]
    ...
    def training_setup_style(self, training_args, decoder_path, mapping_path,photorealistic=False,ode=0,n=100):
        self.D=MultiDiscriminator().cuda()
        # compute the final vgg features for each point
        self.final_vgg_features = self.feature_linear.forward_directly_on_point(self._vgg_features)
        ...
        self.style_transfer = MulLayer(matrixSize=32, adain=True,ode=ode,N=n).cuda()

        if mapping_path:
            try:
                #print('Init mlp from {}'.format(mapping_path))

                mapping_state_dict= torch.load(mapping_path)
                self.style_transfer.mlp.load_state_dict(mapping_state_dict)
            except:
                print('Init mlp from {}'.format(mapping_path))
                mapping_state_dict=torch.load(mapping_path)
                mapping_state_dict.pop('layers.5.weight')
                mapping_state_dict.pop('layers.5.bias')
                self.style_transfer.mlp.load_state_dict(mapping_state_dict,strict=False)
        l = [
            {'params': self.decoder.parameters(), 'lr': 2e-3, "name": "decoder"}
        ]
        ...
        self.optimizer = torch.optim.Adam(l, eps=1e-15)
        l_1 = [
            {'params': self.decoder.parameters(), 'lr': 1e-4, "name": "decoder"},
            {'params': self.style_transfer.mlp.parameters(), 'lr': 1e-4, "name": "style_transfer"},
            {'params': self.style_transfer.rectified_flow_1.parameters(), 'lr': 1e-4, "name": "flow_1"},
            {'params': self.style_transfer.rectified_flow_2.parameters(), 'lr': 1e-4, "name": "flow_2"},
            {'params': self.style_transfer.rectified_flow_3.parameters(), 'lr': 1e-4, "name": "flow_3"},
        ]
        l_D = [ {'params': self.style_transfer.discriminator.parameters(), 'lr': 1e-4, "name": "discriminator"}]
        l_2=[{'params': self.decoder.parameters(), 'lr': 1e-4, "name": "decoder"},{'params': self.D.parameters(), 'lr': 1e-4, "name": "dis"},]


        self.optimizer_1 = torch.optim.Adam(l_1, eps=1e-15)
        self.optimizer_2 = torch.optim.Adam(l_2, eps=1e-15)
        self.optimizer_D = torch.optim.Adam(l_D, eps=1e-15)
